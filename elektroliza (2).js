/* ── CONSTANTS & DATA ── */
  ...
  */
  ...
  const phFactorMap   = {'7.0':0.75, '7.2':0.66, '7.4':0.50, '7.8':0.33};
  ...
  function izracunaj(){ try{
    ...
    let povrsina=0, dubina=0;
    if(oblik==='regular'){
      const sirina=num('sirina'), duzina=num('duzina'); dubina=num('dubinaRegular');
      if(sirina<=0||duzina<=0||dubina<=0) throw new Error('Unesi širinu, dužinu i dubinu pravilnog bazena.');
      povrsina=sirina*duzina;
    } else {
      povrsina=num('povrsinaIrregular'); dubina=num('dubinaIrregular');
      if(povrsina<=0||dubina<=0) throw new Error('Unesi površinu i dubinu nepravilnog bazena.');
    }
    const volumen=povrsina*dubina;
    if(volumen<=0) throw new Error('Volumen bazena mora biti veći od 0.');
    ...
    $('mFaktori').textContent=`${fmt(fo,2)} / ${fmt(fpH,2)} / ${fmt(ft,2)} / ${fmt(fUV,2)}`;
    ...
    renderRows([
      ['Oblik bazena',oblik==='regular'?'Pravilni':'Nepravilni'],
      ['Površina bazena',`${fmt(povrsina,2)} m²`],
      ['Dubina bazena',`${fmt(dubina,2)} m`],
  ...
  function ucitajPrimjer(){
    $('oblikBazena').value='regular';
    $('phVode').value='7.2';
    $('sirina').value=4; $('duzina').value=8; $('dubinaRegular').value=1.5;
    $('povrsinaIrregular').value=32; $('dubinaIrregular').value=1.5;
  ...
  function ocistiSve(){
    $('oblikBazena').value='regular';
    $('phVode').value='7.2';
    $('sirina').value=''; $('duzina').value=''; $('dubinaRegular').value='';
    $('povrsinaIrregular').value=''; $('dubinaIrregular').value='';
    $('temperaturaVode').value='28';
    $('opterecenjeBazena').value='srednje';
    $('izlozenostSuncu').value='visoko';
    $('mVolumen').textContent='-'; $('mFaktori').textContent='-'; $('mRequired').textContent='-';
